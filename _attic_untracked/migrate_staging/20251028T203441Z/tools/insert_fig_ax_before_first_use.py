...
RX_HAS_PLT = re.compile(
    r"^\s*(?:from\s+matplotlib\s+import\s+pyplot\s+as\s+plt|import\s+matplotlib\.pyplot\s+as\s+plt)\b"
)
...
def process(p: Path) -> bool:
    ...
    if first_use is None:
        print("[OK] no fig/ax/plt usage in", p)
        return False
    ...
    has_figax_above = any(
        re.search(r"\b(fig\s*,\s*ax|ax|fig)\s*=\s*plt\.subplots\(", s) for s in L[:ins]
    )
