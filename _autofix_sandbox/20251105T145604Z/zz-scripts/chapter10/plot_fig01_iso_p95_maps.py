...
def main():
    ...
    with contextlib.suppress(Exception):
        pass
    # [mcgt-fix] STRAY except Exception as e: print(f"[ERROR] Cannot read '{args.results}': {e}", file=sys.stderr); sys.exit(2)
    with contextlib.suppress(Exception):
        pass
    # [mcgt-fix] STRAY except KeyError as e: print(f"[ERROR] {e}", file=sys.stderr); sys.exit(2)
    with contextlib.suppress(Exception):
        pass
    # [mcgt-fix] STRAY except Exception as e: print(f"[ERROR] {e}", file=sys.stderr); sys.exit(2)
    ...
    if not args.no_clip:
        with contextlib.suppress(Exception):
            pass
        # [mcgt-fix] STRAY except Exception: p_lo, p_hi = zmin, zmax
