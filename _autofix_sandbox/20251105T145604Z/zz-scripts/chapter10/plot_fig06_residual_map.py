...
# - Colorbar pré-scalée (affiche en x10^exp rad ; défaut exp=-7).
...
# ap.add_argument("--results", required=True, help="CSV d'entrée.")
    ...
    ap.add_argument(
# "--scale-exp", type=int, default=-7, help="Exponent pour l'échelle x10^exp rad."
    )
    ap.add_argument(
        "--threshold",
        type=float,
...
# metric_label = rf"{metric_name}"
    ...
    # Pré-scaling pour l'affichage : valeurs en unités “x10^exp rad”
...
# exp_txt = f"x 10^{args.scale_exp}"  # ex: x 10^-7
