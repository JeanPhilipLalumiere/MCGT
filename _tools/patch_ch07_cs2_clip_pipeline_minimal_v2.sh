...
trap 'code=$?;
  ...
  echo "[ASTUCE] Seul mcgt/scalar_perturbations.py est touché, avec backup .bak_cs2_rewrite_v2.";
  read -rp "Appuie sur Entrée pour revenir au shell..." _' ERR

echo "== PATCH CH07 v2 – Réécriture complète de compute_cs2 (warning + clip) =="
...
backup = path.with_suffix(".py.bak_cs2_rewrite_v2")
...
# 1) Trouver le début de la fonction compute_cs2 (recherche plus tolérante)
start = None
...
if start is None:
    raise SystemExit("[ERROR] Signature de compute_cs2 introuvable (aucune ligne contenant 'def compute_cs2').")

# 2) Trouver la fin : prochaine def ou prochain gros séparateur
end = None
for j in range(start + 1, len(lines)):
    if lines[j].startswith("def ") and "def compute_cs2" not in lines[j]:
        end = j
        break
...
def compute_cs2(k_vals: np.ndarray, a_vals: np.ndarray, p: PertParams) -> np.ndarray:
    ...
    Cette version est assouplie pour le *pipeline minimal* :
...
echo
echo "Terminé (patch_ch07_cs2_clip_pipeline_minimal_v2)."
