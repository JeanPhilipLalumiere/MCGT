...
python3 scripts/09_dark_energy_cpl/generate_data_chapter09.py || echo "[WARN] generate_data a renvoyé un code non nul (on continue pour diagnostiquer)"

# 2) fig01 overlay (après patch meta guard)
echo "[INFO] Étape 2/4: fig01 overlay"
python3 scripts/09_dark_energy_cpl/plot_fig01_phase_overlay.py || echo "[WARN] fig01 a renvoyé un code non nul"

# 3) Builder fig02 input (détecte colonnes IMR/MCGT, aligne fréquences)
echo "[INFO] Étape 3/4: build fig02 input"
python3 tools/build_fig02_input.py
...
python3 scripts/09_dark_energy_cpl/plot_fig02_residual_phase.py --csv "$CSV" --out "$OUT_PNG" --dpi 160 || echo "[WARN] fig02 a renvoyé un code non nul"
