#!/usr/bin/env python3
# add_to_manifest.py — ajoute/actualise/supprime des entrées dans assets/zz-manifests/manifest_master.json
...
def guess_role(p: Path) -> str:
    s = str(p).replace("\\", "/")
    if "/assets/zz-schemas/" in s:
        return "schema"
    if "/assets/zz-manifests/" in s:
        return "manifest"
    if "/config/" in s:
        return "config"
    if "/scripts/" in s:
        return "script"
    if "/assets/zz-figures/" in s:
        return "figure"
    if "/assets/zz-data/" in s or "/zz-donnees/" in s:
        return "data"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Register/update files in assets/zz-manifests/manifest_master.json (repro inventory)."
    )
    ...
    p.add_argument(
        "--manifest",
        default="assets/zz-manifests/manifest_master.json",
        help="manifest JSON path (default: assets/zz-manifests/manifest_master.json)",
    )
    p.add_argument(
        "--role",
        choices=[
