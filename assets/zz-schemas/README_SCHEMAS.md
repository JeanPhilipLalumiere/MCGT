Purpose : Ce document décrit l’ensemble des schémas, les validateurs et les règles de validation globales qui assurent la cohérence de bout en bout sur les dix chapitres MCGT. Il explique quels fichiers existent dans assets/zz-schemas/, comment les utiliser, ce qu’ils valident et comment ils s’intègrent aux tâches QA du Makefile.



Directory : Sauf mention contraire, tous les fichiers cités se trouvent dans assets/zz-schemas/.
...
• validation\_globals.json — constantes canoniques trans-chapitres, seuils, réglages de dérivées, fenêtres de métriques, alias de chemins (zz-donnees→assets/zz-data, chapitre→chapter), normalisation des étiquettes de classes (primaire/ordre2→primary/order2) et localisations d’instances utilisées par les diagnostics.
...
• (hors de ce dossier) diag\_consistency.py — utilise assets/zz-schemas/validation\_globals.json pour exécuter des contrôles transverses (valeurs canoniques, plages, fenêtres, classes, identifiants) et produire un rapport structuré.
...
python diag\_consistency.py --rules assets/zz-schemas/validation\_globals.json --out-json assets/zz-manifests/diag\_consistency\_report.json
...
• Les fichiers de schéma portent des noms en anglais sous assets/zz-schemas/ ; les dossiers de données par chapitre utilisent l’anglais (« chapterXX »). validation\_globals.json fournit des alias afin que les arborescences historiques en français (zz-donnees/chapitreX) soient également valides.
...
• make consistency : exécute diag\_consistency.py et écrit assets/zz-manifests/diag\_consistency\_report.json.
...
• Politique de mise à jour : incrémenter « schema\_version » dans validation\_globals.json lors de changements de règles canoniques ; en cas de modification de structure d’une instance JSON, incrémenter le « $id » du schéma correspondant et consigner le changement dans assets/zz-manifests/manifest\_report.md.
