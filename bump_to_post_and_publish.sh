...
python3 -m pip show build >/dev/null 2>&1 || python3 -m pip install -U build >/dev/null

# Version courante depuis pyproject.toml
cur_ver="$(python3 - <<'PY'
...
# Calcul next postrelease
next_ver="$(python3 - <<PY
import re
cur="${cur_ver}"
m=re.search(r'^(\d+)\.(\d+)\.(\d+)(?:\.post(\d+))?$', cur)
if not m: print("", end="")
else:
    M,mn,p,post = m.groups()
    print(f"{M}.{mn}.{p}.post{int(post)+1 if post else 1}", end="")
PY
)"
[[ -n "${next_ver}" ]] || die "Impossible de calculer la postrelease pour ${cur_ver}"
info "new=${next_ver}"

export NEW_VER="${next_ver}"

# Mise à jour pyproject.toml (dans [project])
python3 - <<'PY'
...
# Synchronise __version__ si présent
