#!/usr/bin/env python3
"""Compute Bayesian evidence ln(Z) for LCDM baseline with dynesty (SN+BAO+CMB)."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Nested Sampling evidence run for LCDM baseline (SN+BAO+CMB)."
    )
    ...
    parser.add_argument("--maxiter", type=int, default=None, help="Maximum number of nested iterations.")
    ...
    parser.add_argument("--processes", type=int, default=max(1, mp.cpu_count()))
...
def init_runtime(root: Path, sigma_sys: float, n_steps_int: int) -> None:
    ...
    tri_probe = _load_module(tri_probe_path, "lcdm_tri_probe")
...
def log_likelihood(theta: np.ndarray) -> float:
    ...
    # LCDM baseline inside the same CPL engine by fixing w0=-1, wa=0.
...
def main() -> int:
    ...
    out_pkl = out_dir / "lcdm_nested_res.pkl"
    ...
    report_path = out_dir / "lcdm_nested_evidence_report.txt"
    report_lines = [
        "LCDM Nested Sampling Evidence Report (SN+BAO+CMB)",
        f"lnZ = {lnz:.6f}",
