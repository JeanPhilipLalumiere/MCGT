...
We define $\Delta\chi^2 \equiv \chi^2_{\Psi\mathrm{TMG}} - \chi^2_{\Lambda\mathrm{CDM}}$; hence, negative values strictly indicate an improvement over the standard model. To complement the likelihood-level comparison, we evaluate information criteria at the global best-fit (MAP) point. For the information criteria evaluation, we define the free parameter count as $k_{\Psi\mathrm{TMG}} = 5$ and $k_{\Lambda\mathrm{CDM}} = 3$, explicitly accounting for the shared nuisance parameters analytical marginalization across both models. Using the full data vector, we compute
\[
...
Confrontation with the Planck likelihood surface. The exact best-fit (MAP) point lies within the confidence region.
