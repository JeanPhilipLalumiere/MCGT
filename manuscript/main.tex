...
\hypersetup{
    ...
    filecolor=magenta,
    urlcolor=cyan,
    pdftitle={Psi-CDM v3.0.0 on the Metric-Coupled Gravity Theory framework},
...
\pagestyle{fancy}
\fancyhf{}
\rhead{\small Psi-CDM v3.0.0 -- "The Great Reconciliation"}
\lhead{\small J.-P. Lalumi\`ere}
\cfoot{\thepage}

\title{\textbf{\Huge $\Psi$CDM Cosmology on the MCGT Framework}\\[0.5cm] \Large A geometric unification for cosmological tensions ($H_0$, JWST, $S_8$)}
\author{\textbf{Jean-Philip Lalumi\`ere} \\ Theoretical Cosmology Laboratory}
\date{Version 3.0.0 -- February 2026}
...
To map this scalar-tensor dynamic to observational constraints, the effective dark energy in the $\Psi$CDM realization is modeled phenomenologically by the Chevallier-Polarski-Linder (CPL) parametrization:
\begin{equation}
    w(a) = w_0 + w_a (1 - a) \quad \text{where} \quad a = \frac{1}{1+z}
\end{equation}
The audited optimal values (\textbf{Best-Fit v3.0.0}) are:
...
\begin{equation}
    \frac{H^2(z)}{H_0^2} = \Omega_r (1+z)^4 + \Omega_m (1+z)^3 + \Omega_{\Psi\mathrm{CDM}} \exp\left[3 \int_0^z \frac{1+w(z')}{1+z'} dz'\right]
\end{equation}
...
The log-log calibration indicates that the $\Psi$CDM realization can reproduce the Planck initial conditions ($A_s, n_s$) without excessive fine-tuning.
...
\begin{figure}[H]
    ...
    \caption{\textbf{Phase space stability map.} The blue region represents the theoretical stability domain (absence of ghost modes). The red line traces the evolution of the $\Psi$CDM Universe from the Big Bang to the present day.}
\end{figure}
...
\begin{figure}[H]
    ...
    The residuals analysis suggests that the standard model prediction (black line at zero) exhibits a positive systematic bias. The $\Psi$CDM dynamics (blue curve) follows the trend of observational data toward lower luminosity distances. \textit{\footnotesize (Data: Pantheon+. Script: pipeline/plots.py. Commit: v3.0.0)}}
    \label{fig:residuals}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Hubble parameter $H(z)$.} Expansion comparison. The $\Psi$CDM curve (blue) reaches $H_0 \approx 73$ km/s/Mpc, in agreement with local data (SH0ES gray points), while $\Lambda$CDM (orange) remains lower ($\approx 67$). \textit{\footnotesize (Data: SH0ES, BOSS DR12. Script: pipeline/plots.py. Commit: v3.0.0)}}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Big Bang Nucleosynthesis (BBN).} Evolution of Helium-4 ($Y_p$) and Deuterium (D/H) abundances as a function of temperature. The $\Psi$CDM predictions (solid lines) remain compatible with the standard model.}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Origin of early galaxies.} Comparison of the linear structure growth rate $f(z)$ between $\Psi$CDM (blue) and $\Lambda$CDM (orange). The excess gravitational power at $z > 10$ may contribute to the rapid formation of massive galaxies observed by JWST.}
    \label{fig:growth}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Expansion and BAO.} Fit of the normalized Hubble parameter to BAO data (BOSS DR12, eBOSS). The $\Psi$CDM model intersects the Lyman-$\alpha$ data points at high redshift ($z \approx 2.3$).}
\end{figure}
...
$\Psi$CDM adjusts $H(z)$ prior to recombination to maintain $100\theta^* \approx 1.04$, which can help reduce the $H_0$ tension.
...
To ensure the improvements are driven by the metric-coupling dynamics and not merely the added degrees of freedom, the analysis was also run in a strict wCDM limit ($w_a = 0$), confirming the statistical preference for the highly dynamic $\Psi$CDM baseline.
...
To complement the likelihood-level comparison, we evaluate information criteria at the global best-fit point. Using $k=5$ free parameters for the baseline $\Psi$CDM run and the full data vector, we compute
\[
\mathrm{AIC} = \chi^2 + 2k, \qquad \mathrm{BIC} = \chi^2 + k \ln n .
\]
Relative to $\Lambda$CDM, we obtain $\Delta \mathrm{AIC} = -145.6$ and $\Delta \mathrm{BIC} = -129.24$. Both values indicate strong model-selection support in favor of the $\Psi$CDM baseline, beyond a pure goodness-of-fit effect.
...
\begin{figure}[H]
    ...
    The upper panel shows the spectra for $\Psi$CDM (blue) and $\Lambda$CDM (orange).
    The lower panel (ratio) indicates a power suppression of about 10\% at small scales ($k > 1 h/\mathrm{Mpc}$), consistent with gravitational lensing constraints. \textit{\footnotesize (Data: Planck 2018 Lensing. Script: pipeline/plots.py. Commit: v3.0.0)}}
...
\begin{figure}[H]
    ...
    \caption{\textbf{CMB temperature spectrum (residuals).} Difference between the $\Psi$CDM theoretical model and Planck 2018 data. The residuals remain consistent with cosmic noise. \textit{\footnotesize (Data: Planck 2018 TT,TE,EE. Script: pipeline/plots.py. Commit: v3.0.0)}}
\end{figure}

\newpage
\section{Synthesis: Addressed Tensions and Implications}

The $\Psi$CDM model proposes a unified reading of observational discrepancies, bridging constraints from multiple probes.

\begin{itemize}
    \item \textbf{Hubble Tension ($H_0$):} $H_0^{\Psi\mathrm{CDM}} = 72.97^{+0.32}_{-0.30}$ km/s/Mpc. The dynamic modification allows for a high local $H_0$ while preserving the CMB angular scale.
    ...
    \item \textbf{Lensing Tension ($S_8$):} $S_8^{\Psi\mathrm{CDM}} = 0.718 \pm 0.030$. The suppression of the high-frequency power spectrum (\autoref{fig:power_spectrum}) reduces the disagreement with Weak Lensing.
\end{itemize}
...
\begin{figure}[H]
    ...
    Center: the $\Psi$CDM model (blue) overlaps both domains, illustrating a possible statistical reconciliation of the probes.}
    \label{fig:tensions}
\end{figure}
