...
\hypersetup{
    ...
    filecolor=magenta,
    urlcolor=cyan,
    pdftitle={MCGT v2.7.2: Metric-Coupled Gravity Theory},
...
\pagestyle{fancy}
\fancyhf{}
\rhead{\small MCGT v2.7.2 -- "The Great Reconciliation"}
...
\date{Version 2.7.2 -- February 2026}
...
\end{equation}
The audited optimal values (\textbf{Best-Fit v2.7.2}) are:
...
\begin{figure}[H]
    ...
    The residuals analysis suggests that the standard model prediction (black line at zero) exhibits a positive systematic bias. The MCGT dynamics (blue curve) follows the trend of observational data toward lower luminosity distances. \textit{\footnotesize (Data: Pantheon+. Script: pipeline/plots.py. Commit: v2.7.2)}}
    \label{fig:residuals}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Hubble parameter $H(z)$.} Expansion comparison. The MCGT curve (blue) reaches $H_0 \approx 73$ km/s/Mpc, in agreement with local data (SH0ES gray points), while $\Lambda$CDM (orange) remains lower ($\approx 67$). \textit{\footnotesize (Data: SH0ES, BOSS DR12. Script: pipeline/plots.py. Commit: v2.7.2)}}
\end{figure}
...
\begin{figure*}[t]
    ...
    \caption{Marginalized 1D and 2D posterior distributions (68\% and 95\% confidence levels) for the primary MCGT cosmological parameters ($\Omega_m, H_0, w_0, w_a, S_8$). The analysis demonstrates a natural convergence toward $H_0 = 73.00 \pm 0.30$ km s$^{-1}$ Mpc$^{-1}$, effectively resolving the $H_0$ tension. Furthermore, the strong negative correlation between $w_0$ and $w_a$ indicates a highly dynamic dark energy sector, providing the geometric framework necessary for early massive galaxy formation as observed by JWST.}
...
\begin{figure}[H]
    ...
    The lower panel (ratio) indicates a power suppression of about 10\% at small scales ($k > 1 h/\mathrm{Mpc}$), consistent with gravitational lensing constraints. \textit{\footnotesize (Data: Planck 2018 Lensing. Script: pipeline/plots.py. Commit: v2.7.2)}}
...
\begin{figure}[H]
    ...
    \caption{\textbf{CMB temperature spectrum (residuals).} Difference between the MCGT theoretical model and Planck 2018 data. The residuals remain consistent with cosmic noise. \textit{\footnotesize (Data: Planck 2018 TT,TE,EE. Script: pipeline/plots.py. Commit: v2.7.2)}}
\end{figure}
