...
\begin{figure}[H]
    ...
    The residuals analysis suggests that the standard model prediction (black line at zero) shows a mild residual trend. The $\Psi$TMG dynamics (blue curve) follows the trend of observational data toward lower luminosity distances. \textit{\footnotesize (Data: Pantheon+. Script: pipeline/plots.py. Commit: v3.1.0)}}
    \label{fig:residuals}
\end{figure}
...
\begin{table}[htbp]
    ...
    {\footnotesize Note: $\sigma_8$ is the directly sampled free parameter, while $S_8 \equiv \sigma_8 (\Omega_m / 0.3)^{0.5}$ is reported here as a derived parameter for cosmological comparison.}
