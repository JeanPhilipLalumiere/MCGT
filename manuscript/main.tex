...
\pagestyle{fancy}
\fancyhf{}
\rhead{\small $\Psi$TMG v3.1.0 -- Simultaneous Resolution of Cosmological Tensions}
...
\begin{figure}[H]
    ...
    The data show a mild redshift-dependent residual trend relative to the $\Lambda$CDM zero-residual baseline. The $\Psi$TMG dynamics (blue curve) follows the trend of observational data toward lower luminosity distances. \textit{\footnotesize (Data: Pantheon+. Script: pipeline/plots.py. Commit: v3.1.0)}}
    \label{fig:residuals}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{CMB temperature spectrum (residuals).} Difference between the $\Psi$TMG theoretical model and Planck 2018 data. Note that the full Planck power spectrum likelihood is not directly sampled during the MCMC inference, which relies on the compressed CMB distance prior. The TT/TE/EE residual spectra shown here serve as a posterior diagnostic and sanity check to confirm that the primordial acoustic structure remains undisturbed. The residuals remain consistent with cosmic variance and measurement noise. \textit{\footnotesize (Data: Planck 2018 TT,TE,EE. Script: pipeline/plots.py. Commit: v3.1.0)}}
\end{figure}
...
\subsection{4.1 Robustness to the Equation of State}
