...
echo "[GIT] Diff rapide:"
git --no-pager diff -- "$WF" || true
echo
read -r -p "Commit & push manifest-guard v7 ? [y/N] " ans
if [[ "$ans" =~ ^[Yy]$ ]]; then
  git add "$WF"
  git commit -m "ci(manifest-guard): v7 — fix YAML indentation for run: blocks [SAFE $(date -u +%Y%m%dT%H%M%SZ)]"
  git push
  echo "[GIT] Pushed."
fi
