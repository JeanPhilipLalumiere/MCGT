# repo_add_ch09_fig03_safe_runner.sh
set -Eeuo pipefail
TS="$(date +%Y%m%dT%H%M%S)"
LOG="/tmp/mcgt_ch09_fig03_safe_${TS}.log"
exec > >(tee -a "$LOG") 2>&1
...
SAFE="zz-scripts/chapter09/run_fig03_safe.py"
OUTPNG="zz-figures/chapter09/09_fig_03_hist_absdphi_20_300.png"
...
echo "== DRY-RUN =="
TMP="/tmp/mcgt_ch09_fig03_${TS}"
mkdir -p "$TMP"
if [ -f "zz-data/chapter09/09_phase_diff.csv" ]; then
  python "$SAFE" --diff zz-data/chapter09/09_phase_diff.csv --out "$TMP/09_fig_03_hist_absdphi_20_300.png" --dpi 150 || true
  ls -lh "$TMP" || true
else
  echo "[NOTE] zz-data/chapter09/09_phase_diff.csv introuvable — testera fallback --csv si fourni."
fi
...
echo "== COMMIT (runner uniquement) =="
git add "$SAFE" || true
git commit -m "ch09/fig03: runner sûr (prefer --diff; fallback --csv; fenêtre 20–300 Hz; x-log)" || true
