...
wait_one(){
  ...
  while [ $tries -lt 30 ]; do
    if [ $have_jq -eq 1 ] && [ -n "$SHA" ]; then
      id="$(gh run list --workflow="$wf" --limit 10 --json databaseId,headSha \
           -q "map(select(.headSha==\"$SHA\")) | .[0].databaseId" 2>/dev/null || true)"
    else
      id="$(gh run list --workflow="$wf" --branch "$REF" --limit 1 --json databaseId -q '.[0].databaseId' 2>/dev/null || true)"
    fi
    [ -n "${id:-}" ] && break
  ...
  local timeout=600 step=10 waited=0 status="" conc=""
  while true; do
    if [ $have_jq -eq 1 ]; then
      status="$(gh run view "$id" --json status,conclusion -q '.status' 2>/dev/null || true)"
      conc="$(gh run view "$id" --json status,conclusion -q '.conclusion' 2>/dev/null || true)"
    else
...
if [ $have_jq -eq 1 ]; then
  for wf in "${WF_LIST[@]}"; do
    gh run list --workflow="$wf" --limit 1 --json name,status,conclusion,headBranch,headSha,htmlUrl \
      | jq -r '.[] | "\(.name) — \(.status)/\(.conclusion) — \(.headBranch) — \(.headSha[0:12]) — \(.htmlUrl)"' | tee -a "$LOG"
  done
else
