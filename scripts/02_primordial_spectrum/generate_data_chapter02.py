#!/usr/bin/env python3
"""
Pipeline d'intégration chronologique corrigé pour Chapter 2 (MCGT)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Pipeline Chapter 2 (+ option spectre primordial)"
    )
    parser.add_argument(
        "--spectre",
        action="store_true",
...
def main(spectre=False):
    ...
    DATA_DIR = ROOT / "assets/zz-data" / "chapter02"
    IMG_DIR = ROOT / "assets/zz-figures" / "chapter02"
    ...
    if spectre:
        ...
        subprocess.run(
            [
                "python3",
                str(ROOT / "scripts" / "chapter02" / "plot_fig00_spectrum.py"),
            ],
            check=True,
        )
