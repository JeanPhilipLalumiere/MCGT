...
def main() -> None:
    ...
    default_results = root / "assets/zz-data" / "chapter10" / "10_results_global_scan.csv"
    default_out = root / "assets/zz-figures" / "chapter10" / "10_fig_01_iso_p95_maps.png"
    default_config = root / "configuration" / "mcgt-global-config.ini"

    ap = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        description="Figure 01 – Carte iso de p95 (m1 vs m2) pour l'exploration globale (Chapter 10).",
    )
    ap.add_argument(
        "--results",
        default=None,
    ...
    ap.add_argument(
        ...
        help="Fichier INI pour récupérer le best-fit CPL (w0, wa, Omega_m).",
    )
    ap.add_argument(
        "--best-fit-x",
        type=float,
        default=None,
        help="Coordonnée X explicite pour le marqueur best-fit (override w0).",
    )
    ap.add_argument(
        "--best-fit-y",
        type=float,
        default=None,
        help="Coordonnée Y explicite pour le marqueur best-fit (override wa).",
    )
    ap.add_argument(
        "--levels",
        type=int,
    ...
    # Marqueur best-fit CPL (tri-sondes) sur la carte
    explicit_bf = args.best_fit_x is not None or args.best_fit_y is not None
    bf_x = args.best_fit_x
    bf_y = args.best_fit_y
    bf_om = None
    bf_label = "Best-Fit tri-sondes"
    if bf_x is None or bf_y is None:
        ...
        if cfg_path.exists():
            ...
            cfg.read(cfg_path)
            if cfg.has_section("dark_energy"):
                sec = cfg["dark_energy"]
                bf_x = bf_x if bf_x is not None else sec.getfloat("w0", fallback=None)
                bf_y = bf_y if bf_y is not None else sec.getfloat("wa", fallback=None)
            for sec_name in cfg.sections():
                sec = cfg[sec_name]
                if bf_om is None:
                    bf_om = sec.getfloat("omega_m", fallback=None)
        if bf_om is not None:
            bf_label = f"Best-Fit tri-sondes (Ωm={bf_om:.3f})"
    in_bounds = (
        bf_x is not None
        and bf_y is not None
        and (x_lo <= bf_x <= x_hi)
        and (y_lo <= bf_y <= y_hi)
    )
    if not in_bounds and not explicit_bf:
        idx = int(np.nanargmin(z))
        bf_x = float(x[idx])
        bf_y = float(y[idx])
        bf_label = "Best-Fit (scan)"
        in_bounds = True
    if bf_x is not None and bf_y is not None:
        if not in_bounds:
            print(
                "[WARN] Best-fit marker outside map bounds; use --best-fit-x/--best-fit-y to override.",
                file=sys.stderr,
            )
        ax.scatter(
            [bf_x],
