...
def evaluate_sample(
    ...
    sid = int(row["id"])
    result.update({
        "id": sid, "m1": float(row["m1"]), "m2": float(row["m2"]),
    ...
    try:
        # 1) Calculer phi_ref via backend et ALIGNER à 0
        ...
        # --- FIX : On force la référence à commencer à 0.0 ---
        phi_ref = phi_ref_raw - phi_ref_raw[0]

        # 2) Calculer phi_mcgt et ALIGNER à 0
        if phi_mcgt is None:
        ...
        # --- FIX : On aligne aussi le modèle à 0.0 ---
        ...
        result.update({
            "k": int(k), "mean_20_300": met["mean"], "p95_20_300": met["p95"],
            "max_20_300": met["max"], "n_20_300": int(met["n"]),
            "status": "ok", "error_code": "", "wall_time_s": float(time.time() - t0),
            "model": row.get("model", "default"), "score": float(met["p95"]),
        })
        return result

    except Exception as e:
        result.update({
            "status": "failed", "error_code": str(e)[:20],
            "wall_time_s": float(time.time() - t0), "score": float("nan"),
        })
        return result
