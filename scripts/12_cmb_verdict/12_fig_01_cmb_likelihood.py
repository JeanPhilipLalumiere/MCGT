...
def load_config(path: Path) -> dict[str, float]:
    ...
    de = cfg["dark_energy"] if "dark_energy" in cfg else None
    ...
    return {
        ...
        "omega_m": omega_m,
        "omega_r": omega_r,
        "w0": None if de is None else de.getfloat("w0", fallback=None),
        "wa": None if de is None else de.getfloat("wa", fallback=None),
    }
...
def main() -> int:
    ...
    if cfg["w0"] is not None and cfg["wa"] is not None:
        ax.scatter(
            [cfg["w0"]],
            [cfg["wa"]],
            marker="*",
            ...
            edgecolors="black",
            linewidths=0.7,
            label=rf"Best-Fit (Ωm={cfg['omega_m']:.3f})",
            zorder=5,
        )
