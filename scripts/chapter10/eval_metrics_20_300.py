...
Usage (exemple) :
python scripts/chapitre10/eval_metrics_principal_20_300.py \
  --samples assets/zz-data/chapitre10/10_mc_samples.csv \
  --ref-grid assets/zz-data/chapitre9/09_phases_imrphenom.csv \
  --out-results assets/zz-data/chapitre10/10_mc_results.csv \
  --out-best    assets/zz-data/chapitre10/10_mc_best.json \
...
def parse_args(argv=None):
    ...
    p.add_argument(
        "--out-results",
        default="assets/zz-data/chapitre10/10_mc_results.csv",
        help="CSV résultats (sortie)",
    )
    p.add_argument(
        "--out-best",
        default="assets/zz-data/chapitre10/10_mc_best.json",
