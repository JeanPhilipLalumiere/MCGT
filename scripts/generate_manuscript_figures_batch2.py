...
def make_concept_schema():
    ...
    center = patches.FancyBboxPatch((3.5, 2.5), 3, 1.2, boxstyle="round,pad=0.3", **box_style)
    left = patches.FancyBboxPatch((0.8, 2.5), 2.4, 1.2, boxstyle="round,pad=0.3", **box_style)
    right = patches.FancyBboxPatch((6.8, 2.5), 2.4, 1.2, boxstyle="round,pad=0.3", **box_style)
    ...
    ax.add_patch(patches.FancyArrow(3.2, 3.1, 0.3, 0, width=0.02, length_includes_head=True))
    ax.add_patch(patches.FancyArrow(6.8, 3.1, -0.3, 0, width=0.02, length_includes_head=True))
...
def make_sentinel_flowchart():
    ...
    start = add_box(2, 12.0, 6, 1.2, "Start\n(Input Params)", "#f2f2f2")
    t1 = add_box(2, 9.8, 6, 1.2, "Causality Check\n($c_s^2 \\leq 1$)", "#c6dbef")
    t2 = add_box(2, 7.6, 6, 1.2, "Energy Positivity\n($\\rho > 0$)", "#c6dbef")
    t3 = add_box(2, 5.4, 6, 1.2, "Perturbation Stability", "#c6dbef")
    valid = add_box(2, 2.8, 6, 1.2, "Valid Model", "#a1d99b")

    stop1 = add_box(8.4, 9.8, 1.2, 1.2, "Stop", "#fcae91")
    stop2 = add_box(8.4, 7.6, 1.2, 1.2, "Stop", "#fcae91")
    stop3 = add_box(8.4, 5.4, 1.2, 1.2, "Stop", "#fcae91")
    ...
    arrow(8.0, 10.4, 8.4, 10.4)
    arrow(8.0, 8.2, 8.4, 8.2)
    arrow(8.0, 6.0, 8.4, 6.0)

    ax.text(7.5, 10.7, "Fail", ha="right", va="center", fontsize=9)
    ax.text(7.5, 8.5, "Fail", ha="right", va="center", fontsize=9)
    ax.text(7.5, 6.3, "Fail", ha="right", va="center", fontsize=9)
...
def make_phase_space_plot():
    ...
    traj_x = 1.1 * np.sin(0.6 * t)
    traj_y = 1.0 * np.cos(0.6 * t) - 0.2
    ax.plot(traj_x, traj_y, color="#d73027", lw=2.0)
