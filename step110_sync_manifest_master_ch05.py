...
# Cibles CH05 à resynchroniser dans le manifest
targets = [
...
# Détecte automatiquement la liste d’entrées
...
# Patch des entrées ciblées
for rel in targets:
    p = root / rel
    if not p.exists():
        print(f"[WARN] {rel} n'existe pas sur le FS, ignoré.")
        continue
    ...
    for e in entries:
        ...
        print(f"[WARN] {rel} absent du manifest, aucun patch appliqué.")
