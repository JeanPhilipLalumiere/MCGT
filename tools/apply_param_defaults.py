import re, sys, yaml, pathlib, fnmatch, argparse
...
def process_file(path: pathlib.Path):
    ...
    for i, line in enumerate(lines):
        ...
        if key not in args.keys or key not in wanted: continue
        if any(tok in val_raw for tok in ('|','>','{','[','${{')): continue
        try: v_file = yaml.safe_load(val_raw.strip())
        except Exception: v_file = val_raw.strip().strip('\'"')
        v_def = wanted[key]
        if key == "python-version": v_file, v_def = str(v_file), str(v_def)
        if key == "timeout-minutes":
            try: v_file = int(v_file)
            except: pass
            try: v_def = int(v_def)
            except: pass
        if v_file == v_def: continue
    ...
    if diffs:
        print(f"[PATCH] {path}")
        for ln, k, vf, vd in diffs: print(f"  - L{ln}: {k}: {vf!r} -> {vd!r}")
        if args.apply: path.write_text("\n".join(lines)+("\n" if orig and orig[-1]=="" else ""), encoding="utf-8")
...
for t in targets:
    if t.is_dir():
        for f in t.rglob("*.yml"):  c,_ = process_file(f); any_change |= c
        for f in t.rglob("*.yaml"): c,_ = process_file(f); any_change |= c
    else:
        c,_ = process_file(t); any_change |= c
sys.exit(0)
