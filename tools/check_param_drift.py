import sys, re, pathlib, yaml, fnmatch, argparse

parser = argparse.ArgumentParser()
parser.add_argument("--include-archive", action="store_true", help="ne pas ignorer .ci-archive/**")
...
wanted_yaml = {k: canon_default("yaml_key", k, v) for k,v in (cfg.get("yaml_key") or {}).items()}
wanted_fm   = {k: canon_default("frontmatter_key", k, v) for k,v in (cfg.get("frontmatter_key") or {}).items()}
...
def check_front_matter(p: pathlib.Path, wanted: dict):
    ...
    if not txt or not re.match(r"^---\s*$", txt[0]): return
