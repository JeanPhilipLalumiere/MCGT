...
--exit-status renvoie code de sortie du job (0=success, >0 sinon)

if gh run watch --exit-status "${RID}"; then
log "Run ${RID} terminé: success"
else
log "WARN: run ${RID} terminé en failure (on continue pour récupérer les logs/artefacts s'il y en a)"
fi
...
log "WARN: Aucun artefact téléchargé (peut-être un échec de job ou pas d'upload)"
fi

log "Inventaire des .tgz"
find "${RUN_DIR}/artifacts" -type f -name '*.tgz' -maxdepth 3 -print | tee "${RUN_DIR}/artifacts/_list.txt" || true

Extraction si on trouve notre paquet attendu
...
diag.json attendu sous ${EXTRACT_DIR}/sanity-diag/diag.json
