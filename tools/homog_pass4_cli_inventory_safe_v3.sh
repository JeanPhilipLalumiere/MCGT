...
echo "[HOMOG-PASS4-SAFE v3] Inventaire CLI (--help) avec timeout & détection par contenu"
...
# Collecter tous les .py chapitres 01..10
mapfile -t FILES < <(find "$SROOT"/chapter0{1..9} "$SROOT"/chapter10 -type f -name "*.py" | sort)

# Entêtes
echo "# CLI inventory (pass4-safe v3) $(date -u +"%Y-%m-%dT%H:%M:%SZ")" > "$REPORT_TXT"
echo "file,has_argparse,has_parse_args,has_main_guard,has_savefig,has_show,help_status,help_detail" > "$REPORT_CSV"
: > "$FAIL_LIST"

ok=0; fail=0

# Supprimer uniquement les lignes entièrement commentées (pas les commentaires inline)
