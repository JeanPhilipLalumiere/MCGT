...
try:
    ...
    # Options analytiques rencontrées
    ...
    # Nouveaux flags vus dans les erreurs récentes
    _shim.add_argument('--trim', type=float)                      # args.trim
    _shim.add_argument('--minN', dest='minN', type=int)           # args.minN (N majuscule)
...
def inject_after_future(p: Path):
    ...
    if new != txt:
        bak = p.with_suffix(p.suffix + ".bak_shim4")
        if not bak.exists():
            bak.write_text(txt, encoding="utf-8")
        p.write_text(new, encoding="utf-8")
        print(f"[PATCH] {p}: shim args (v4) injecté")
    else:
        print(f"[OK   ] {p}: rien à faire (déjà propre)")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("usage: tools/inject_args_shim_v4.py FILE [FILE...]", file=sys.stderr); sys.exit(2)
    for a in sys.argv[1:]:
        p = Path(a)
        if not p.exists():
