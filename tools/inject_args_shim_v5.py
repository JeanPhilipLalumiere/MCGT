...
try:
    ...
    # Options analytiques rencontrées précédemment
    ...
    # NOUVEAUX FLAGS vus dans tes dernières erreurs
    _shim.add_argument('--B', dest='B', type=int)         # nb d'itérations bootstrap
    _shim.add_argument('--M', dest='M', type=int)         # taille externe (optionnelle)
    _shim.add_argument('--outer', dest='outer', type=int) # fallback si M n'est pas donné
    _shim.add_argument('--cmap', type=str)                # colormap
    ...
    _DEF = {{
        ...
        # Nouveaux
        'B': 2000,
        'M': None,       # important: présent mais None, pour ne pas forcer le code si 'outer' est attendu
        'outer': 500,
        'cmap': 'viridis',
        'zoom_x': 0.0,
        'zoom_dx': 1.0,
...
def inject_after_future(p: Path):
    ...
    if new != txt:
        bak = p.with_suffix(p.suffix + ".bak_shim5")
        if not bak.exists():
            bak.write_text(txt, encoding="utf-8")
        p.write_text(new, encoding="utf-8")
        print(f"[PATCH] {p}: shim args (v5) injecté")
    else:
        print(f"[OK   ] {p}: rien à faire (déjà propre)")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("usage: tools/inject_args_shim_v5.py FILE [FILE...]", file=sys.stderr); sys.exit(2)
    for a in sys.argv[1:]:
        p = Path(a)
        if not p.exists():
