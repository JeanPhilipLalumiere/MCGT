...
try:
    ...
    # Options analytiques déjà vues
    _shim.add_argument('--npoints', type=int)
    ...
    _shim.add_argument('--trim', type=float)
    _shim.add_argument('--minN', dest='minN', type=int)
    _shim.add_argument('--point-size', dest='point_size', type=float)
    _shim.add_argument('--zoom-w', dest='zoom_w', type=float)
    _shim.add_argument('--zoom-h', dest='zoom_h', type=float)
    _shim.add_argument('--abs', action='store_true', dest='abs')
    _shim.add_argument('--p95-ref', dest='ref_p95', type=float)
    _shim.add_argument('--B', dest='B', type=int)
    _shim.add_argument('--M', dest='M', type=int)
    _shim.add_argument('--outer', dest='outer', type=int)
    _shim.add_argument('--cmap', type=str)
    _shim.add_argument('--zoom-x', dest='zoom_x', type=float)
    _shim.add_argument('--zoom-dx', dest='zoom_dx', type=float)
    _shim.add_argument('--scale-exp', dest='scale_exp', type=float)
    # NOUVEAUX (observés dans tes derniers logs)
    _shim.add_argument('--zoom-center-n', dest='zoom_center_n', type=int)
    _shim.add_argument('--inner', dest='inner', type=int)
    _shim.add_argument('--title', dest='title', type=str)
    _shim.add_argument('--zoom-y', dest='zoom_y', type=float)
    _shim.add_argument('--zoom-dy', dest='zoom_dy', type=float)
    _shim.add_argument('--vclip', dest='vclip', type=str)
    ...
    _DEF = {{
        ...
        'abs': False,
        'B': 2000,
        'M': None,          # laissé à None si non fourni
        'outer': 500,
        'cmap': 'viridis',
        'zoom_x': 0.0,
        'zoom_dx': 1.0,
        'scale_exp': 0.0,
        # Nouveaux défauts
        'zoom_center_n': None,  # si None, le code choisira M/2
        'inner': 2000,
        'title': 'MCGT figure',
        'zoom_y': 0.0,
        'zoom_dy': 1.0,
        'vclip': '1,99',   # percentiles typiques
    }}
...
def inject_after_future(p: Path):
    ...
    need_nl_before = (ins != 0 and txt[ins-1] != "\n")
    need_nl_after  = (ins < len(txt) and txt[ins] != "\n")
    new = txt[:ins] + ("\n" if need_nl_before else "") + SHIM + ("\n" if need_nl_after else "") + txt[ins:]
    if new != txt:
        bak = p.with_suffix(p.suffix + ".bak_shim6")
        if not bak.exists():
            bak.write_text(txt, encoding="utf-8")
        p.write_text(new, encoding="utf-8")
        print(f"[PATCH] {p}: shim args (v6) injecté")
    else:
        print(f"[OK   ] {p}: rien à faire")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("usage: tools/inject_args_shim_v6.py FILE [FILE...]", file=sys.stderr); sys.exit(2)
    for a in sys.argv[1:]:
        p = Path(a)
        if not p.exists():
