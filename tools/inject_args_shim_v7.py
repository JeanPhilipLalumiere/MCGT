...
try:
    ...
    # Options analytiques (v6 + ajouts)
    _shim.add_argument('--npoints')
    ...
    _shim.add_argument('--metric')
    _shim.add_argument('--bins')
    _shim.add_argument('--alpha')
    _shim.add_argument('--boot-iters', dest='boot_iters')
    _shim.add_argument('--seed')
    _shim.add_argument('--trim')
    _shim.add_argument('--minN', dest='minN')
    _shim.add_argument('--point-size', dest='point_size')
    _shim.add_argument('--zoom-w', dest='zoom_w')
    _shim.add_argument('--zoom-h', dest='zoom_h')
    _shim.add_argument('--abs', action='store_true', dest='abs')
    _shim.add_argument('--p95-ref', dest='ref_p95')
    _shim.add_argument('--B', dest='B')
    _shim.add_argument('--M', dest='M')
    _shim.add_argument('--outer', dest='outer')
    _shim.add_argument('--cmap')
    _shim.add_argument('--zoom-x', dest='zoom_x')
    _shim.add_argument('--zoom-dx', dest='zoom_dx')
    ...
    _shim.add_argument('--zoom-y', dest='zoom_y')
    _shim.add_argument('--zoom-dy', dest='zoom_dy')
    _shim.add_argument('--vclip', dest='vclip')
    # NOUVEAUX vus dans les logs récents
    ...
    # Casts robustes (si valeurs venues en str)
    def _to_int(x):
        try: return int(x)
        except: return x
    def _to_float(x):
    ...
    for _k in ('dpi','B','M','outer','bins','boot_iters','npoints','minN','inner','zoom_center_n'):
        if hasattr(args,_k): setattr(args,_k, _to_int(getattr(args,_k)))
    for _k in ('alpha','trim','change_eps','point_size','zoom_w','zoom_h','zoom_x','zoom_dx','zoom_y','zoom_dy','scale_exp','hist_scale','threshold'):
...
def inject_after_future(p: Path):
    ...
    if new != txt:
        bak = p.with_suffix(p.suffix + ".bak_shim7")
        if not bak.exists(): bak.write_text(txt, encoding="utf-8")
        p.write_text(new, encoding="utf-8")
        print(f"[PATCH] {p}: shim args (v7) injecté")
    else:
        print(f"[OK   ] {p}: rien à faire")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("usage: tools/inject_args_shim_v7.py FILE [FILE...]", file=sys.stderr); sys.exit(2)
    for a in sys.argv[1:]:
        p = Path(a)
        if not p.exists():
