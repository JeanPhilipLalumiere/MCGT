...
try:
    ...
    # Nouveaux vus/nécessaires dans les logs
    ...
    _shim.add_argument('--figsize')  # ex: "6,5"
    ...
    _DEF = {{
        ...
        'hist_scale': 1.0,
        'threshold': 0.0,
        'title_left': 'Left panel',
        'title_right': 'Right panel',
        'hist_x': 0.0,
        'hist_y': 0.0,
        'figsize': '6,4',
    }}
...
def inject_after_future(p: Path):
    ...
    if new != txt:
        bak = p.with_suffix(p.suffix + ".bak_shim8")
        if not bak.exists(): bak.write_text(txt, encoding="utf-8")
        p.write_text(new, encoding="utf-8")
        print(f"[PATCH] {p}: shim args (v8) injecté")
    else:
        print(f"[OK   ] {p}: rien à faire")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("usage: tools/inject_args_shim_v8.py FILE [FILE...]", file=sys.stderr); sys.exit(2)
    for a in sys.argv[1:]:
        p = Path(a)
        if not p.exists():
