...
if ! python3 scripts/chapter09/generate_data_chapter09.py; then
...
python3 scripts/chapter09/plot_fig01_phase_overlay.py || echo "[WARN] fig01 a signalé une erreur"
...
if [[ -n "${CSV_CANDIDATE:-}" ]]; then
  ...
  python3 scripts/chapter09/plot_fig02_residual_phase.py --csv "$CSV_CANDIDATE" --out "$OUT_PNG" || echo "[WARN] fig02 a signalé une erreur"
else
