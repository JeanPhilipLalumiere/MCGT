...
def update_manifest(path: Path, name: str, version: str) -> None:
    ...
    print(f"[OK] Mis à jour {path} → project.name={project.get('name')}, project.version={project.get('version')}")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--version", required=True, help="Nouvelle version de projet (ex: 0.2.99)")
    parser.add_argument("--name", default=None, help="Nom du projet (ex: mcgt-core). Laisse vide pour ne pas changer.")
