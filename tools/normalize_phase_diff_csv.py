...
ACT = pick(
    df, ["phi_active", "phi_mcgt_cal", "phi_mcgt", "phi_model", "phi_mcgt_active"]
)
...
if F is not None:
        ...
        try:
            ...
            if (
                ...
                if len(f) != len(df):
                    ...
                    df = df.iloc[:n, :].copy()
                print(
                    f"[INFO] f reconstruit depuis metrics: [{f[0]:.3f},{f[-1]:.3f}] n={len(f)}"
                )
    ...
    if f is None:
        ...
        if ref_csv.exists():
            ...
            Fref = pick(
                dfref,
                ["f", "freq", "frequency_Hz", "Frequency_Hz", "Frequency", "f_Hz"],
            )
            if Fref:
        ...
        if f is None:
            sys.exit(
                "Colonne fréquence absente et impossible à reconstruire (metrics/IMR)."
            )
...
# Cas 4 : dernier recours -> prendre deux colonnes "phi*"
    ...
    if len(phase_like) >= 2:
        ...
        sys.exit(
            "Impossible de normaliser : colonnes phi_ref/phi_active/dphi manquantes."
        )
