...
set -euo pipefail
echo "[PASS11b] Dé-stubifier ch01–02, purger tight_layout, re-valider (--help)"

CHAPS=( "zz-scripts/chapter01" "zz-scripts/chapter02" )
INVENTORY="tools/homog_pass4_cli_inventory_safe_v5.sh"
[[ -x "$INVENTORY" ]] || { echo "[ERR] $INVENTORY introuvable"; exit 1; }

restored=0
for D in "${CHAPS[@]}"; do
  echo "[PASS11b] Recherche de stubs PASS6 dans $D ..."
...
echo "[PASS11b] Restaurations: $restored"

echo "[PASS11b] Purge tight_layout (rect + simple) dans ch01–02…"
for D in "${CHAPS[@]}"; do
  while IFS= read -r -d '' f; do
    safe_cp "$f" "$f.pass11b" 2>/dev/null || true
...
echo "[PASS11b] Vérif: aucun tight_layout actif (hors lignes commentées)…"
...
echo "[OK] Aucun tight_layout actif en ch01–02."

# Re-scan inventaire (tolérant v5)
"$INVENTORY"

# Si des FAIL ch01–02 subsistent, insérer un guard léger PASS5B puis re-scan
FAIL_LIST="zz-out/homog_cli_fail_list.txt"
if grep -qE '^zz-scripts/(chapter01|chapter02)/' "$FAIL_LIST" 2>/dev/null; then
  echo "[PASS11b] Guards PASS5B sur FAIL ch01–02…"
  python3 - <<'PY'
import pathlib,re
...
targets=[pathlib.Path(l.strip()) for l in fail_list.read_text().splitlines()
         if l.strip().startswith(("zz-scripts/chapter01/","zz-scripts/chapter02/"))]
for p in targets:
