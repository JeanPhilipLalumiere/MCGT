...
CHAPS=( "zz-scripts/chapter05" "zz-scripts/chapter04" "zz-scripts/chapter03" "zz-scripts/chapter08" )
INVENTORY="tools/homog_pass4_cli_inventory_safe_v5.sh"
[[ -x "$INVENTORY" ]] || { echo "[ERR] $INVENTORY introuvable"; exit 1; }

restored=0
for D in "${CHAPS[@]}"; do
  echo "[PASS11d] Recherche de stubs PASS6 dans $D ..."
...
echo "[PASS11d] Restaurations: $restored"
...
for D in "${CHAPS[@]}"; do
  while IFS= read -r -d '' f; do
    ...
    # fig.tight_layout() -> subplots_adjust
    perl -0777 -pe '
...
echo "[PASS11d] Vérif: aucun tight_layout actif (hors lignes commentées)…"
...
echo "[OK] Aucun tight_layout actif en ch05–04–03–08."
...
if grep -qE '^zz-scripts/(chapter05|chapter04|chapter03|chapter08)/' "$FAIL_LIST" 2>/dev/null; then
  echo "[PASS11d] Guards PASS5B sur FAIL ch05–04–03–08…"
  python3 - <<'PY'
import pathlib,re
...
targets=[pathlib.Path(l.strip()) for l in fail_list.read_text().splitlines()
         if l.strip().startswith(("zz-scripts/chapter05/","zz-scripts/chapter04/","zz-scripts/chapter03/","zz-scripts/chapter08/"))]
for p in targets:
