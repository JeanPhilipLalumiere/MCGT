...
echo "[PASS9] Force-stub des FAIL restants + re-scan inventaire"

FAIL_LIST="zz-out/homog_cli_fail_list.txt"

# Si la fail-list est absente ou vide, on relance un inventaire pour la produire
if [[ ! -s "$FAIL_LIST" ]]; then
...
# Toujours rien ? => rien à faire
[[ -s "$FAIL_LIST" ]] || { echo "[PASS9] Aucun FAIL — rien à faire."; exit 0; }

# Injection de stubs temporaires dans chaque FAIL (idempotent, conserve .bak)
python3 - <<'PY'
...
print(f"[PASS9] Fichiers stubifiés: {patched}")
PY

# Re-scan inventaire pour confirmer
tools/homog_pass4_cli_inventory_safe_v4.sh
...
# Vérifier qu'il ne reste aucun tight_layout actif (sécurité globale)
