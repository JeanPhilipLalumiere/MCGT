...
OUT_DIR = Path(".ci-out"); OUT_DIR.mkdir(exist_ok=True, parents=True)
        ...
        if re.search(r"^\s*uses\s*:\s*actions/setup-python@v\d+\s*$", line):
            ...
            while j < len(lines) and (lines[j].strip() == "" or lines[j].startswith((" ", "\t"))):
                if re.search(r"^\s*with\s*:\s*$", lines[j]):
                    ...
                    k = j + 1
                    found = False
                    while k < len(lines) and (lines[k].startswith((" ", "\t")) and not re.match(r"^\s*\S", lines[k].lstrip())):
                    ...
                    if not found:
                        ...
                        pyver_changes.append((wf, j+1, "(insert python-version: '3.12')"))
                    break
                j += 1
...
with PLAN.open("w", encoding="utf-8") as f:
    ...
    if timeout_hits:
        f.write("== Ajouter `timeout-minutes: 15` à ces workflows (au niveau des jobs sans timeout) ==\n")
        for p in timeout_hits:
            f.write(f"- {p}\n")
        f.write("\n")
    else:
        f.write("== Tous les workflows contiennent déjà au moins un `timeout-minutes` ==\n\n")

    if pyver_changes:
        f.write("== Forcer/insérer `python-version: '3.12'` dans les steps setup-python suivants ==\n")
