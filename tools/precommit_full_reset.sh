...
echo "==> (2) ÉCRASE .pre-commit-config.yaml avec un squelette valide (LF, espaces)"
...
if ! grep -qE '^[[:space:]]*\.ci-out[[:space:]]+export-ignore[[:space:]]*$' .gitattributes; then
  # assure une fin de ligne avant d'ajouter
...
echo "==> (4) .gitignore : ajoute '.ci-out/' (idempotent) + désindexe .ci-out/"
...
echo "==> (5) Remet les permissions d’exécution sur tools/*.sh (si présents)"
...
git commit -m "ci: reset pre-commit config; exclude .ci-out; add export-ignore; fix exec bits" || true
git push || true
