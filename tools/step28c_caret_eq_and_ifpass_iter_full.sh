...
echo "[STEP28c] 0) Smoke de référence"
tools/pass14_smoke_with_mapping.sh

echo "[STEP28c] 1) Cibler: cannot-assign / '== ?' / expected body after if"
awk -F, 'NR>1{
...
}' "$CSV" | sort -u > zz-out/_step28c_targets.lst || true
wc -l zz-out/_step28c_targets.lst

# Write the robust fixer as a separate, small Python file (avoids heredoc truncation).
cat > tools/_eq_ifpass_fix.py <<'PY'
...
targets = [t for t in sorted(set(TARGET_LIST.read_text(encoding="utf-8").splitlines())) if t and Path(t).exists()]
...
def insert_pass_if_needed(lines, head_start_i, head_end_j):
    ...
    need_pass = (k >= len(lines) or
                 ...
                 NEXT_KWS.match(lines[k]) is not None)
    if need_pass:
...
print(f"[RESULT] eq_changes={tot_eq} pass_inserted={tot_pass} files_changed={changed_files}")
PY

echo "[STEP28c] 2) Exécution du fixer"
python3 tools/_eq_ifpass_fix.py

echo "[STEP28c] 3) Re-smoke + top erreurs"
tools/pass14_smoke_with_mapping.sh
