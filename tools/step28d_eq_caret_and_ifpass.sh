...
echo "[STEP28d] 0) Smoke de référence"
tools/pass14_smoke_with_mapping.sh

echo "[STEP28d] 1) Cibler: cannot-assign / '== ?' / expected body after if"
awk -F, 'NR>1{
...
}' "$CSV" | sort -u > zz-out/_step28d_targets.lst || true
wc -l zz-out/_step28d_targets.lst

# Write the fixer in two short chunks to avoid heredoc truncation.
truncate -s 0 tools/_eq_ifpass_fix_min.py
cat >> tools/_eq_ifpass_fix_min.py <<'PY'
from pathlib import Path
import re

T = Path("zz-out/_step28d_targets.lst")
if not T.exists():
    print("[STEP28d] no targets"); raise SystemExit(0)
targets = [t for t in sorted(set(T.read_text(encoding="utf-8").splitlines())) if t and Path(t).exists()]
...
cat >> tools/_eq_ifpass_fix_min.py <<'PY'
    ...
    if k<0: return False
    return (txt[k].isalnum() or txt[k] in '._)]')
        ...
        try:
            ...
            if "expected an indented block after 'if' statement" in msg or \
               ...
               "expected an indented block after 'while' statement" in msg:
                ...
                if insert_pass_if_needed(lines,i,end): pc+=1; made=True; continue
            break
            ...
            if ("cannot assign to expression here" in msg) or ("Maybe you meant '=='" in msg) or \
               ("invalid syntax. Maybe you meant '=='" in msg) or ("invalid syntax. Maybe you meant '==' or ':='" in msg):
                ...
                if end is None: break
                rel=col
                for k in range(i, min(end+1, ln)): rel += len(lines[k])
...
for f in targets:
    ok, eqa, pa = process(f)
    if ok: 
        changed+=1; tot_eq+=eqa; tot_pass+=pa; print(f"[FIX] {f}")
print(f"[RESULT] eq_changes={tot_eq} pass_inserted={tot_pass} files_changed={changed}")
PY

echo "[STEP28d] 2) Exécution du fixer (line count sanity)"
wc -l tools/_eq_ifpass_fix_min.py
python3 tools/_eq_ifpass_fix_min.py

echo "[STEP28d] 3) Re-smoke + top erreurs"
tools/pass14_smoke_with_mapping.sh
