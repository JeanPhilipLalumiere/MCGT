...
chi2vals.append( c1 + c2)

pd.DataFrame({"T_Gyr": T, "chi2_nucleosynthesis": chi2vals}).to_csv(CHI2FILE, index=False)
# 6) Dérivée et lissage de χ2
dchi2raw = np.gradient( chi2vals, T)
# Fenêtre impair ≤ 21 pour éviter oversmoothing
win = min( 21( len( dchi2raw ) // 2 ) * 2 + 1)
dchi2smooth = savgol_filter( dchi2raw, win, polyorder=3, mode="interp")
pd.DataFrame({"T_Gyr": T, "dchi2_smooth": dchi2smooth}).to_csv( DERIV_FILE, index=False)
