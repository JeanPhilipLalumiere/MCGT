...
def load_config(ini_path: Path) -> PhaseParams:
    ...
    # découpe
    x_split = float(s.get("x_split", cfg["segmentation"].getfloat("x_split")))

    # lissage (section [lissage] ou fallback vers [scan])
    if "lissage" in cfg:
        lissage = cfg["lissage"]
        window = int(lissage.get("derivative_window", lissage.get("window")))
        polyord = int(lissage.get("derivative_polyorder", lissage.get("polyorder")))
    else:
...
def parse_args():
    ...
    p.add_argument(
        "--export-2d", action="store_true", help="Exporter aussi les matrices 2D"
    )
    p.add_argument(
        "--n-k", type=int, metavar="NK", help="Override du nombre de points en k"
    )
    p.add_argument(
        "--n-a", type=int, metavar="NA", help="Override du nombre de points en a"
    )
    p.add_argument(
        "--dry-run", action="store_true", help="Valide config et grille sans calcul"
    )
    p.add_argument(
        "--log-level",
        default="INFO",
    ...
    p.add_argument("--log-file", metavar="FILE", help="Fichier de log additionnel")
    return p.parse_args()
...
def main():
    ...
    if "k" not in raw.columns:
        ...
        if "k" not in raw.columns:
            ...
            num_cols = [
                c for c in raw.columns if _np.issubdtype(raw[c].dtype, _np.number)
            ]
            if num_cols:
                raw = raw.rename(columns={num_cols[0]: "k"})
            else:
                raw = raw.copy()
                raw["k"] = _np.arange(len(raw))
    ...
    phi_i = make_interpolator_linear(
        df_slice["k"], df_slice["delta_phi_raw"] * p.delta_phi_param
    )(k_grid)
    ...
    # export dérivées
    pd.DataFrame({"k": k_grid, "d_cs2_dk": dcs2}).to_csv(
        DATA_DIR / "07_dcs2_dk.csv", index=False
    )
    pd.DataFrame({"k": k_grid, "d_delta_phi_dk": dphi}).to_csv(
        DATA_DIR / "07_ddelta_phi_dk.csv", index=False
    )
    ...
    ).to_csv(DATA_DIR / "07_scalar_perturbations_results.csv", index=False)
    logger.info(
        "Perturbations scalaires (résultats) exportées → 07_scalar_perturbations_results.csv"
    )

    # invariants
    I1 = cs2_i / k_grid
    pd.DataFrame({"k": k_grid, "I1_cs2": I1}).to_csv(
        DATA_DIR / "07_scalar_invariants.csv", index=False
    )
    logger.info("Invariants exportés")

    # domaine
    pd.DataFrame(
        [{"k_min": p.k_min, "k_max": p.k_max, "a_min": 0.0, "a_max": p.a_max}]
    ...
    pd.DataFrame([{"k_split": p.x_split, "cs2_at_split": cs2_at}]).to_csv(
        DATA_DIR / "07_perturbations_boundary.csv", index=False
    )
    ...
    _ = globals().get("ROOT", Path.cwd())
    meta_path.write_text(json.dumps(meta, indent=2), encoding="utf-8")
    logger.info("Méta-JSON écrit → %s", meta_path)

    # export matrices 2D si demandé
    if args.export_2d:
        raw[["k", "a", "cs2_raw"]].rename(columns={"cs2_raw": "cs2_matrix"}).to_csv(
            DATA_DIR / "07_cs2_matrix.csv", index=False
        )
        raw[["k", "a", "delta_phi_raw"]].rename(
            columns={"delta_phi_raw": "delta_phi_matrix"}
