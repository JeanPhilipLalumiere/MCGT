...
def parse_args():
    ...
    p.add_argument(
        "--q0star_min", type=float, required=True, help="Valeur minimale de q0⋆"
    )
    p.add_argument(
        "--q0star_max", type=float, required=True, help="Valeur maximale de q0⋆"
    )
    p.add_argument(
        "--n_points",
        type=int,
    ...
    p.add_argument(
        "--n_param2", type=int, default=50, help="Nombre de points pour le 2ᵈ paramètre"
    )
    return p.parse_args()
...
def main():
    ...
    pd.DataFrame({"z": zbao, "DV_calc": dvb}).to_csv(
        DATA_DIR / "08_dv_theorie_z.csv", index=False
    )
    ...
    pd.DataFrame({"z": zsn, "mu_calc": mub}).to_csv(
        DATA_DIR / "08_mu_theorie_z.csv", index=False
    )
    ...
    if args.export_derivative:
        ...
        pd.DataFrame({"q0star": q0, "dchi2_smooth": ds}).to_csv(
            DATA_DIR / "08_derivee_chi2.csv", index=False
        )
