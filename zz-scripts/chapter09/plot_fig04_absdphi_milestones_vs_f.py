...
def parse_args():
    ap = argparse.ArgumentParser(
        description="fig_04 – |Δφ|(f) + jalons (principal, calage cohérent)"
    )
    ap.add_argument(
        "--diff",
        type=Path,
    ...
    ap.add_argument(
        "--jalons", type=Path, default=DEF_JALONS, help="CSV jalons (requis)."
    )
    ...
    ap.add_argument(
        "--show_autres", action="store_true", help="Afficher les jalons 'autres'."
    )
    ap.add_argument(
        "--apply-calibration",
        choices=["auto", "on", "off"],
...
def main():
    ...
    raw_cls = (
        J.get("classe", pd.Series(["autres"] * len(J)))
        .astype(str)
        .str.lower()
        .str.replace(" ", "")
        .str.replace("-", "")
    ...
    log.info(
        "xlim=[%.3g, %.3g] Hz ; ylim=[%.3g, %.3g] rad ; N_jalons=%d",
        xmin,
        xmax,
        ymin,
        ymax,
