...
# -*- coding: utf-8 -*-
"""
10_fig_03b_convergence_zoom.py

CH10 – Figure 03b: convergence zoom (minimum N=10) with CI coverage and width.
English only. Defaults to p95_rad, outputs PNG + manifest for fig07.
...
def main() -> None:
    ...
    p.add_argument(
        "--out",
        default="zz-figures/chapter10/10_fig_03b_convergence_zoom.png",
        help="Output PNG",
    )
    p.add_argument(
        "--outer",
    ...
    p.add_argument(
        "--title-left",
        default="Convergence (minimum N=10)",
        help="Left panel title.",
    )
    p.add_argument(
        "--title-right",
        default="Mean CI width vs N",
        help="Right panel title.",
    )
    # options ajoutées
    p.add_argument(
        "--hires2000",
    ...
    if args.angular and (ref_value_circ is not None):
        ...
        inset.set_title("Reference N=max", fontsize=9)
