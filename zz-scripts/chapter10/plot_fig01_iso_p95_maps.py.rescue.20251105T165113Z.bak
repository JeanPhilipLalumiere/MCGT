...
if hint and hint in df.columns:
        raise SystemExit(hint)  # auto-rescue: was return at module level
candidates = [
"p95_20_300_recalc",
"p95_20_300_circ",
"p95_20_300",
...
for c in candidates:
        if c in df.columns:
            raise SystemExit(c)  # auto-rescue: was return at module level
for c in df.columns:
...
raise SystemExit(df)  # auto-rescue: was return at module level
...
# auto-rescue: commented → x1 = x[tris[:, 0]]
# auto-rescue: commented → x2 = x[tris[:, 1]]
# auto-rescue: commented → x3 = x[tris[:, 2]]
# auto-rescue: commented → y1 = y[tris[:, 0]]
# auto-rescue: commented → y2 = y[tris[:, 1]]
# auto-rescue: commented → y3 = y[tris[:, 2]]
# auto-rescue: commented → areas = 0.5 * np.abs((x2 - x1) * (y3 - y1) - (x3 - x1) * (y2 - y1))
# auto-rescue: commented → mask = areas <= 0.0
        # triang.set_mask expects boolean mask with same length as triangles
# auto-rescue: commented → triang.set_mask(mask)
except Exception:
        pass
try:
        pass
        ...
        pass
raise SystemExit(triang)  # auto-rescue: was return at module level
...
# auto-rescue: commented → if False:  # auto-rescue: orphan except Exception
            # auto-rescue: commented → pass
# auto-rescue: commented → try:
            # auto-rescue: commented → pass
# auto-rescue: commented → if False:  # auto-rescue: orphan except Exception
# auto-rescue: commented → if p_hi - p_lo > 1e-8 and (p_lo > zmin or p_hi < zmax):
            # auto-rescue: commented → vmin, vmax, clipped = float(p_lo), float(p_hi), True
# auto-rescue: commented → warnings.warn(f"Clipping [{vmin:.4g}, {vmax:.4g}] (0.1%–99.9%).")
# auto-rescue: commented → norm = colors.Normalize(vmin=vmin, vmax=vmax, clip=True)
# auto-rescue: commented → plt.style.use("classic")
# auto-rescue: commented → fig, ax = plt.subplots(figsize=(10, 8))
# auto-rescue: commented → cf = ax.tricontourf(triang, z, levels=levels, cmap=args.cmap, alpha=0.95, norm=norm)
# auto-rescue: commented → _cs = ax.tricontour(
