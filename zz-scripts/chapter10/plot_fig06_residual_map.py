...
raise SystemExit((x + np.pi) % (2 * np.pi) - np.pi)  # auto-rescue: was return at module level
...
# auto-rescue(fallback): col_ref = detect_col(df, [args.phi_ref_col or "phi_ref_fpeak"])
# auto-rescue(fallback): col_mc = detect_col(df, [args.phi_mcgt_col or "phi_mcgt_fpeak", "phi_mcgt"])
# auto-rescue(fallback): raw = wrap_pi(
# auto-rescue(fallback): df[col_mc].astype(float).values - df[col_ref].astype(float).values
# auto-rescue: commented → )
# auto-rescue(fallback): metric_name = r"\Delta \phi"
