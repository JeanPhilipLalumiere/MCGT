import sys, pandas as pd, numpy as np, pathlib as p, re

IMR = p.Path("zz-data/chapter09/09_phases_imrphenom.csv")
MCGT = p.Path("zz-data/chapter09/09_phases_mcgt.csv")
...
def best_freq_col(df):
    cand = [["f"], ["freq"], ["frequency_hz"], ["frequency"], ["f_hz"], ["nu"], ["nu_hz"]]
    c = pick(df, cand)
    if c: return c
    ...
    for c in nums:
        ...
        if np.all(np.isfinite(x)) and np.all(np.diff(x) > 0): return c
...
def find_phi_col(df, priority_regexes, fallback_regex=r"phi|phase"):
    ...
    numeric = [c for c in cols if pd.api.types.is_numeric_dtype(df[c]) and c.lower() not in freq_names]
    if numeric:
        numeric = sorted(
            numeric,
            key=lambda c: pd.to_numeric(df[c], errors="coerce").std(skipna=True),
...
phi_imr = find_phi_col(
    df_imr,
    priority_regexes=[r"^phi_?ref(_cal|_raw)?$", r"^phi_?imr$", r"ref.*phi", r"imr.*phi"]
)
if not phi_imr:
...
phi_mc = find_phi_col(
    df_mc,
    priority_regexes=[
        r"^phi_?active$", r"^phi_?mcgt(_active|_cal)?$", r"^phi(_cal)?$",
        r"^phase(_mcgt|_active)?$", r"mcgt.*phi", r"active.*phi"
    ]
)
if not phi_mc:
    sys.exit("Aucune colonne de phase MCGT détectée (essayé: phi_active/phi_mcgt/phi/phase...).")
...
f_match = fm[idx]
ok = np.isfinite(f_match) & (np.abs(f_match - fi) <= np.maximum(1e-6, 1e-6 * np.maximum(1.0, fi)))
...
out = out[np.isfinite(out["f"]) & np.isfinite(out["phi_ref"]) & np.isfinite(out["phi_active"])]

OUT.parent.mkdir(parents=True, exist_ok=True)
out.to_csv(OUT, index=False)
print(f"[OK] fig02_input.csv → {OUT} (n={len(out)}) ; IMR({f_imr},{phi_imr}) vs MCGT({f_mc},{phi_mc})")
