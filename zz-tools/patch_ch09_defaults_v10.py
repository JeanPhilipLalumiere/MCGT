...
if "_mcgt_safe_float" not in src:
    print("[ERREUR] Helper _mcgt_safe_float absent. (Assure-toi que V9 est appliqué.)", file=sys.stderr)
    sys.exit(3)
...
src, n3 = re.subn(r'float\(\s*np\.nanmin\(\s*f\s*\)\s*\)', '_mcgt_safe_float(np.nanmin(f), 20.0)', src)
src, n4 = re.subn(r'float\(\s*np\.nanmax\(\s*f\s*\)\s*\)', '_mcgt_safe_float(np.nanmax(f), 300.0)', src)
n_changes += n3 + n4
