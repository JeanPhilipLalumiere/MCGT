...
src = re.sub(r"(?s)#\s*=== MCGT Hotfix:.*?def _mcgt_safe_float\(.*?return float\(default\)\s*\n\s*\n", "", src)
...
while i < len(lines) and (is_shebang(lines[i]) or is_coding(lines[i]) or is_blank(lines[i])):
...
if i < len(lines) and is_doc_start(lines[i]):
    ...
    if doc[0].lstrip().count(q) == 1:
        while i < len(lines):
            doc.append(lines[i])
            if q in lines[i]: i += 1; break
            i += 1

# futures consécutifs
futures = []
while i < len(lines) and is_future(lines[i]):
    futures.append(lines[i]); i += 1
...
def reinforce(txt):
    ...
    for k, dv in {"m1":30.0,"m2":25.0,"fmin":20.0,"fmax":300.0}.items():
        subn(rf'float\(\s*cfg\s*\[\s*["\']{re.escape(k)}["\']\s*\]\s*\)', f'_mcgt_safe_float(cfg.get("{k}"), {dv})')
        subn(rf'float\(\s*cfg\s*\.?\s*get\(\s*["\']{re.escape(k)}["\']\s*\)\s*\)', f'_mcgt_safe_float(cfg.get("{k}"), {dv})')
        subn(rf'^(\s*{re.escape(k)}\s*=\s*)float\((.*?)\)\s*$', rf'\1_mcgt_safe_float(\2, {dv})', flags=re.MULTILINE)
    # durcir nanmin/nanmax sur fmin/fmax dans helpers internes
    subn(r'float\(\s*np\.nanmin\(\s*f\s*\)\s*\)', '_mcgt_safe_float(np.nanmin(f), 20.0)')
    subn(r'float\(\s*np\.nanmax\(\s*f\s*\)\s*\)', '_mcgt_safe_float(np.nanmax(f), 300.0)')
    return txt, total
