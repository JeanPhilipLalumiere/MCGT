...
Cette version garde une API propre pour les notebooks/plots du chapitre 02:
- A_s(alpha), n_s(alpha), P_R(k, alpha)
- generate_spec() écrit un JSON simple de méta-données.

Objectif: débloquer le formatage (ruff-format) et la CI pendant la Phase 0.
"""
...
# Paramètres jouets (stables) — à recalibrer en Phase 1+
A_S0 = 2.1e-9
NS0 = 0.965
C1 = 0.5
C2 = -0.02
...
def P_R(k: np.ndarray, alpha: float) -> np.ndarray:
    ...
    k : array-like > 0
    alpha : borne dans [-0.1, 0.1] pour Phase 0
    """
    alpha = float(alpha)
    if not (-0.1 <= alpha <= 0.1):
        raise ValueError("alpha doit être dans [-0.1, 0.1] en Phase 0.")
...
def generate_spec() -> None:
    """Écrit un JSON de méta-données simple pour la CI."""
    spec = {
